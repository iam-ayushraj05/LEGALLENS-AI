...
export const LegalDisclaimer: React.FC<LegalDisclaimerProps> = ({ variant = 'banner' }) => {
  if (variant === 'inline') {
    return (
      <div className="flex items-start gap-2.5 p-3 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-600" role="note" aria-label="Legal safety notice">
  ...
  if (variant === 'footer') {
    return (
      <footer className="border-t border-slate-200 bg-slate-900 text-slate-400 text-xs py-8 px-4 mt-auto" role="contentinfo">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <ShieldAlert className="w-4 h-4 text-emerald-400" aria-hidden="true" />
  ...
  return (
    <aside className="bg-slate-900 border-b border-slate-800 text-slate-300 text-xs py-2 px-4" aria-label="Disclaimer notice banner">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-2">
        ...
        <a href="#disclaimer-modal" className="text-amber-400 hover:underline shrink-0 text-[11px]" aria-label="Read full safety notice">
          Safety Notice
        </a>
      </div>
    </aside>
